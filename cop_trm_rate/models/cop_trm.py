...
class trmColombian(models.Model):
    ...
    def _get_soap_trm(self):
        ...
        report = {
            ...
            '1. Tasa de Cambio Peso Colombiano/1.1 TRM - Disponible desde '\
            'el 27 de noviembre de 1991/1.1.3 Serie historica para un rango '\
            'de fechas dado',\
            'reportXml': 'null'
        }
        options = {
        ...
        try:
            result_query = client.service.executeXMLQuery(
                report, 'SAWRowsetData', options, session_id
                )
            ...
            _logger.warning('Error while fetching info from BancoRep API: ' 
                            + detail)
    ...
    def get_colombian_trm(self):
        ...
        try:
            old_trm = self.search([('currency_id','=',currency_id)], limit=1,
                order='id desc')[0].rate
        except:
            old_trm = 0
